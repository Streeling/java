...
. In `CustomerRepository` clic pe denumirea clasei, kbd:[Alt+Enter] si alege _Create Test_.
In ferestra _Create Test_ bifeaza _setUp/@Before_ si alege `findById(id:Integer):Customer` din lista, apoi btn:[OK].
In clasa `CustomerRepositoryTest` (generata automat de IDEA) definit campul `repository` (`CustomerRepository`) cu
visibilitatea `private`. Iar in metoda adnotata cu `@BeforeEach` initializati acest camp. Redenumeste metoda `findById`
...
celealte campuri alege valori dupa dorinta, epeleaza metoda `sava()` cu acest obiect, apoi apeleaza `findById(1)` si
...
celealte campuri alege valori dupa dorinta, epeleaza metoda `sava()` cu acest obiect, apoi apeleaza `findById(customer.getId())` si
adauga cate o asertiune pentru fiecare camp:
+
[source, java]
----
...
. Scrie un test pentru metoda `save` care verifica modul in care este generata valoarea `id`.

. Scrie un test pentru `findByAllFields()`.

. Scrie alt test pentru `findByAllFields()`.
