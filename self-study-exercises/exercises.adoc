...
----
+
Iar in `CustomerController` la metoda `customer()` adauga in lista de parametri `Model model`, schimba tipul de intoarcere din `Customer` in `String`, ianuntrul metodei, 
in loc de `return customer;` vom returna valoarea `"customer"` (de fapt e numele fisierului _customer.html_ fara extensie) si in final ianinte de a returna aceasta valoare vom pune variabila `customer` pe model 
